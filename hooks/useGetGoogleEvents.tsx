...
// Define the hook
export const useGetGoogleEvents = () => {
  const { user } = useUser();
  const [events, setEvents] = useState<any[]>([]); // Specify the type here
    ...
    const fetchEvents = async () => {
      ...
      try {
        ...
        const response = await calendar.events.list({
          ...
          maxResults: 100, // Example: Limit to 100 events
          singleEvents: true,
          orderBy: 'startTime',
        });
        ...
        setEvents(events || []); // Ensure items is not undefined
      } catch (error) {
